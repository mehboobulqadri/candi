...
fn tiny_pdf_opens_with_cached_page_count() {
    ...
    let doc = open(BackendKind::Pdfium, path, None).unwrap();
...
fn missing_file_is_not_found() {
    assert!(matches!(
        open(BackendKind::Pdfium, "/no/such/candi-fixture.pdf", None),
...
fn encrypted_without_password_is_encrypted() {
    ...
    assert!(matches!(
        open(BackendKind::Pdfium, &path, None),
...
fn encrypted_with_wrong_password_is_wrong_password() {
    let path = encrypted_fixture().to_str().unwrap().to_string();
    assert!(matches!(
        open(BackendKind::Pdfium, &path, Some("bad")),
...
fn encrypted_with_correct_password_opens() {
    let path = encrypted_fixture().to_str().unwrap().to_string();
    let doc = open(BackendKind::Pdfium, &path, Some("123456")).unwrap();
    assert_eq!(doc.page_count(), 1);
}
...
fn truncated_pdf_may_fail_before_zero_page_guard() {
    ...
    assert!(matches!(
        open(BackendKind::Pdfium, path_str, None),
