...
- **Reading position (sidecar).** `book.pdf.candi.toml` next to the PDF, never the PDF
  itself. Schema v1, versioned from first release; atomic writes (temp file + rename).
  ...
  Written on quit and on page change; a missing sidecar is fine (fresh start); v0.2 grows
  chapters/bookmarks (schema v2) per Spike 4.
...
Semantic tokens, not widget keys, so one theme works across TUI and GUI:
`background`, `foreground`, `muted`, `heading`, `subheading`, `accent`, `link`, `code`,
`border`, `selection`, `warning`, `error`, `success`, `page-background`, `page-text`.
Modes: normal, dark, warm (v0.2; v0.1 ships no themes).
